...
/*
	...
	@param point_val the point value of for this data record
*/
CREATE PROCEDURE create_record(IN `device` int(32), IN `liter_val` int(32), IN `point_val` int(32))
	BEGIN
	IF EXISTS (SELECT *
			FROM `devices` as d
			WHERE d.`device_id` = `device`) THEN
		INSERT INTO `data_records` (`record_liters`, `device_id`, `record_point_value`)
		VALUES (`liter_val`, `device`, `point_val`);
	END IF;
	END //
...
/*
	remove a data record with the given id
	@param id the id of the data record to delete
*/
CREATE PROCEDURE remove_record(IN `id` int(32))
	BEGIN
	IF EXISTS(SELECT *
		FROM `data_records` as dr
		WHERE dr.`record_id` = `id`) THEN
